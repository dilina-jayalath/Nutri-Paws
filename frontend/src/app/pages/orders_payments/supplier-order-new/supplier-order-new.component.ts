...
@Component({
  selector: 'app-supplier-order-new',
  templateUrl: './supplier-order-new.component.html',
  styleUrls: ['./supplier-order-new.component.scss'],
})
export class SupplierOrderNewComponent {
  ...
  getOrderItemsByUserId() {
    ...
    this.api.getAllSupplierOrder().subscribe({
      next: (response: any) => {
        this.orders = response;
        console.log('this ordder' + this.orders);
      },
      error: (error: any) => {
        Swal.fire('Error While Gettig Customer Orders');
      },
  ...
  viewOrder(order: any) {
    this.api.getSupOrderitems(order.orderId).subscribe({
      next: (response: any) => {
  ...
  updateOrderStatus(order: any, newStatus: string) {
    ...
    this.api.updateSupOrderStatus(order.orderId, jsonData).subscribe({
      next: (res: any) => {
  ...
  statusOptions: string[] = ['Place', 'Delivered', 'Returnd'];
}
